...
def resize_by_height(image, height):
    ...
    if new_width < w_min:
        ...
        ret = cv2.copyMakeBorder(
            ret, 0, 0, pad, pad, borderType=cv2.BORDER_CONSTANT, value=(255, 255, 255)
        )
...
def resize_by_width(image, width):
    ...
    if new_height < h_min:
        ...
        ret = cv2.copyMakeBorder(
            ret, pad, pad, 0, 0, borderType=cv2.BORDER_CONSTANT, value=(255, 255, 255)
        )
...
def add_overlays(to_add, height=h_extra_max, position="topleft", placer=None):
    ...
    if len(to_add):
        final_data_image = cv2.copyMakeBorder(
            combined,
            h_extra_max,
            0,
            0,
    ...
    if show_curr_track == True:
        # X, Y
        try:
            curr_track_img = frame2[
                max(0, curr_pts[1] - curr_track_vis) : min(
                    frame2.shape[0], curr_pts[1] + curr_track_vis
                ),
                max(0, curr_pts[0] - curr_track_vis) : min(
                    frame2.shape[1], curr_pts[0] + curr_track_vis
                ),
            ]
...
while True:
    ...
    contours = cv2.findContours(
        contourfinder, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )[0]
    ...
    for i, cnt in enumerate(contours):
        ...
        padded_digit = np.pad(
            resized_digit, ((5, 5), (5, 5)), "constant", constant_values=255
        )
    ...
    def save():
        ...
        cv2.imwrite(
            "/Birinder/Programs/Python/Edge AI/Project2/Exports/" + f"{filename}",
            final_data_image,
        )
